...
| **允许** | 放行这一次请求。 |
| **拒绝** | 拒绝这一次请求。 |
| **返回** | 关掉弹窗,把跑这个 session 的终端标签页带回最前,再交回 Claude Code 的原生提示——"始终允许"(don't ask again)在那里,按程序、按目录的分级远比弹窗精确。 |

按 **Esc** 或直接关掉弹窗,效果同**返回**:回落到原生提示,**永不自动放行**。

不管它,它就一直挂在屏幕上等你回答——没有几秒钟就消失的超时,人不在电脑前泡杯咖啡的功夫请求也不会自己消失。唯一的上限是 Claude Code 自己的 hook 超时,大约两小时;超过之后弹窗会放弃,回落到原生提示,效果同**返回**。
...
- 只有明确点**允许**才放行,**拒绝**才驳回。其余(返回 / Esc / 关闭)一律回落到原生提示——弹窗**从不自动放行,也从不写入任何规则**。
